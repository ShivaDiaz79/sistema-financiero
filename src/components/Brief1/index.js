...
const questions = Object.entries(questionOptions).map(([q, opts]) => ({ question: q, options: opts }));
...
export default function Brief1() {
  ...
  useEffect(() => {
    const loadBriefs = async () => {
      ...
      setSubmittedBriefs(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    };
    loadBriefs();
  }, []);
  ...
  const handleOtherChange = (question, value) => {
    setResponses(prev => ({
      ...prev,
      [question]: [...(prev[question] || []).filter(v => !v.startsWith('Otro:')), `Otro: ${value}`]
    }));
  };
          ...
          <div className="flex flex-wrap gap-2">
            {options.map((opt) => (
              <button
                ...
                className={`px-3 py-1 rounded-full border ${responses[question]?.includes(opt) ? 'bg-black text-white' : 'bg-gray-200 text-gray-800'}`}
              >
                {opt}
              </button>
            ))}
      ...
      {showBriefs && submittedBriefs.length > 0 && (
        <div className="mt-6 space-y-6">
          {submittedBriefs.map((brief) => (
            <div key={brief.id} className="p-6 border rounded-md shadow-md bg-white">
              {Object.entries(brief).map(([q, a]) => (
                <div key={q} className="mb-2">
                  <p className="font-semibold text-gray-700">{q}</p>
                  <p className="text-gray-800 text-sm">{Array.isArray(a) ? a.join(', ') : a}</p>
                </div>
              ))}
